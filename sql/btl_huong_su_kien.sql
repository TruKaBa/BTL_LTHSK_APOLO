...
create proc selecthiensotctichluy
@x int, @y int
as
if(@x > @y)
begin
...
-- Tạo trigger cập nhật điểm kết thúc học phần với fDiemKTHP = 0.1*fDiemCC + 0.2*fDiemGK + 0.7*fDiemThi
create trigger capnhatdiemkthp
on tblHoc
for insert, update
as
begin
	...
	begin
		...
		declare  @diemcc float, @diemgk float, @diemthi float
		select @magv = sMaGV, @mamon = sMaMon, @masv = sMaSV,
		@diemcc = fDiemCC, @diemgk = fDiemGK, @diemthi = fDiemThi from inserted
...
drop proc insertlophanhchinh
exec insertlophanhchinh
...
begin
UPDATE tblLopHC
	SET iSiSo = (select count(sv.sMaSV)
	from tblSinhVien as sv
	where sv.sLopHC = tblLopHC.sMaLop
	group by sLopHC)
	where exists (select tblSinhVien.sLopHC
	from tblSinhVien 
	where tblSinhVien.sLopHC = tblLopHC.sMaLop)
end
...
insert into tblSinhVien
values ('21IT50', 'nguyen van a', '2000/09/09', 'ha noi', 'nam', 'lp1', '0123445555555555', 0)
...
begin
	insert into tblSinhVien
	values(@masv, @tensv, @ngaysinh, @diachi, @gioitinh, @lophc, @sdt)
end
...
begin
	update tblSinhVien
	set sTenSV = @tensv, dNgaySinh = @ngaysinh, sDiaChi = @diachi, sGioiTinh = @gioitinh, sLopHC = @lophc, sSDT = @sdt
	where sMaSV = @masv
end

create proc search_sinhvien
@masv varchar(50), @tensv nvarchar(50)
as
begin
	select * from tblSinhVien
	where sMaSV like '%' + @masv + '%'
	or sTenSV like '%' + @tensv + '%'
end
...
create proc Update_LopMon
@masv varchar(50), @mamon varchar(50), @magv varchar(50),
@dvhv varchar(50), @diemcc float, @diemgk float, @diemthi float
as
begin
	update tblHoc
	set fDiemCC = @diemcc,
...
alter proc select_giangvien_khoa
as
...
update tblHoc
set fDiemKTHP = 0.1 * fDiemCC  + 0.2 * fDiemGK + 0.7 * fDiemThi


-- cập nhật tín chỉ cho sinh viên trc khi cập nhật thì phải reset tất cả tín chỉ về 0 trc
create proc updateTinchiChoSv
as
	declare @masinhvien varchar(50)
	declare @mamon varchar(50)
	DECLARE myCursor CURSOR FOR
		select hoc.sMaSV, hoc.sMaMon
		from tblHoc as hoc
		group by hoc.sMaSV, hoc.sMaMon
		having max(hoc.fDiemKTHP) > 4.0
	open myCursor
	FETCH NEXT FROM myCursor
	INTO @masinhvien, @mamon
	WHILE @@FETCH_STATUS = 0
	begin 
		declare @sotin int
		select @sotin = mon.iSoTC
		from tblMonHoc as mon
		where mon.sMaMon = @mamon
		print 'time '+ ' ' +  @masinhvien + ' ' + @mamon + ' '  + CONVERT(varchar(10), @sotin) 
		update tblSinhVien
		set iTcTichLuy = iTcTichLuy + @sotin
		where sMaSV = @masinhvien
		FETCH NEXT FROM myCursor
		INTO @masinhvien, @mamon
	end
	CLOSE myCursor
...
--trigger cập nhật tính chỉ cho sinh viên khi thêm sửa xóa bảng tblHoc
